#include "elkscoringcardb.hpp"
#include "scoring/scoreutils.hpp"

std::vector<double> ElkScoringCardB::computeScore(const PlayerBoard& board) const {

    int final_score = 0;
    ScoreUtils::TileGrid candidates;
    std::vector<int> points;
    std::vector<int> points_lookup = { 0, 2, 5, 9, 13 };
    ScoreUtils::getShapeFormations(board, MAX_SIZE, Elk, candidates, points, points_lookup);
